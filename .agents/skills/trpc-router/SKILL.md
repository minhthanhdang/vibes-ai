...
## How to load a skill from this router

The skills below are installed but deliberately not registered, so only this
router appears in the skill list. They are not callable via the Skill tool.

To load one, read its file directly:

    .agents/skills/<skill-name>/SKILL.md

So `-> Load skill: server-setup` means read `.agents/skills/server-setup/SKILL.md`.

Load only the skills the current task needs. A few carry extra files in a
`references/` subdirectory (e.g. `.agents/skills/links/references/link-options.md`);
read those only when the SKILL.md points to them.
