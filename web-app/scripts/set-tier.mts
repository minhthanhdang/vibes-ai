...
const [email, asked] = process.argv.slice(2);
if (!email || !asked) {
  console.error("usage: set-tier <email> <TIER_1|TIER_2|TIER_3>");
  process.exit(1);
}
...
const after = await db.user.update({
  where: { email },
  data: { tier: asked as keyof typeof AccountTier },
  select: { email: true, tier: true, vibesBoardsUsed: true },
});
console.log(`${after.email}: ${before.tier} -> ${after.tier} (boards used ${after.vibesBoardsUsed})`);
await closeDb();
