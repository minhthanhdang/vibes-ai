...
/// entirely the ceilings and the sentences that report them. Two of them, and
/// they bound different things — `SKILLS_PER_CALL` is what one answer may
/// carry, `SKILLS_PER_DESIGN` is what the whole design may read over any number
/// of calls — because what a skill costs is characters that then sit in every
/// subsequent request unwindowed (§III.1). A third call is not a new cost; a
/// twentieth skill is.
///
...
export const NO_SKILL_NAMED =
  "name at least one skill to read, by a name from the catalogue in this tool's description. Nothing was read and nothing was spent, so ask again with a name on it.";
...
export type DesignerSkillToolset = {
  ...
  /// allowance is already kept in, offered rather than rebuilt: parsing
  /// `get_skill`'s arguments a second time somewhere else would count a name
  /// the model mistyped, a name over `SKILLS_PER_CALL` and a call the ceiling
  /// refused as skills this design was taught.
  read: () => string[];
};

export function skillToolset(): DesignerSkillToolset {
  /// What this design has read, which is what makes the allowance an allowance
  /// and what a repeated name is checked against. Held per toolset and the
  /// toolset is built per `design_page` call, so "a turn" and "a design" are the
  /// same span here (§VI opens one).
  const read: string[] = [];

  function readSkills(args: Record<string, unknown>): Record<string, unknown> {
    if (read.length >= SKILLS_PER_DESIGN) return { error: skillCeilingSaid(read) };
    ...
    /// would spend the design's allowance on text already in the transcript,
    /// and the whole point of the allowance is that it buys new writing.
    const alreadyRead = asked.filter((name) => read.includes(name));
    const fresh = asked.filter((name) => !read.includes(name));

    /// Both ceilings in one place: a call reads at most `SKILLS_PER_CALL`, and
    /// never more than the design has left.
    const room = Math.min(SKILLS_PER_CALL, SKILLS_PER_DESIGN - read.length);
    const wanted = fresh.slice(0, room);
    const notRead = fresh.slice(room);
    ...
    return {
      ...
      ...(notRead.length && {
        notRead,
        notReadNote: skillsOverCallSaid(SKILLS_PER_DESIGN - read.length),
      }),
    };
  }
