import { arrangementBounds } from "./board-place";
import { boardItems } from "./board-contents";
import { DROPPED_IMAGE_GAP } from "./moodboard-drop";
import { LAYOUT_TEXT_MAX_FONT, LAYOUT_TEXT_MIN_FONT } from "./moodboard-layouts";
import { TEXT_LINE_HEIGHT } from "./moodboard-compose";
import type { SceneElement } from "./moodboard-scene";

/// Putting a line of text on a board the director arranged themselves, and taking
...
/// template from the block count and writes it over an arrangement the director
/// made by hand. A headline is not a reason to lose the board.
///
/// Nothing here is open to judgement. The words are the director's, and a line
...
export type LineResult = {
  ...
  /// repeating something the director said rather than something the board says,
  /// and only the director can tell which line they meant.
  notOnBoard: string[];
  alreadyOn: string[];
};
...
/// `originalText` is what the director typed. A board is read through both, for
...
/// the director resizes type by dragging it, which is the answer to any guess
...
/// that goes, so a headline the director duplicated leaves once.
///
...
export function placeLinesOnBoard({
  ...
  /// words, so two blocks saying the same thing are two things the director
...
/// one word blown up to a title is a deliberate thing a director does and should
