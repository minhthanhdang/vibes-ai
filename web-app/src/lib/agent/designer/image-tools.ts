...
export const DESIGNER_GENERATE_IMAGE: ToolDeclaration = {
  name: "generate_image",
  description: `Draw a picture that is not in this project and file it in the gallery. This is for the ask no upload answers — a paper texture, a wash or a colour field to stand behind a page, a dusk gradient, a plain backdrop, a shape nobody photographed. For subjects, prefer a picture the user already has: a photograph that fits is a photograph somebody chose. For a page's ground it leans the other way — a drawn texture, wash or gradient usually serves a page better than a flat colour, so reach for one when the gallery holds nothing fit to stand behind the page. What comes back is an ordinary gallery image with an id, and put_on_canvas places it on the next round of this same turn. The property analyzer reads it minutes behind, and until it does its line in list_gallery carries the description it was drawn at, so there is nothing to wait for. One picture per call and at most ${GENERATE_CALL_LIMIT} a turn. Say in your closing line that the picture was made rather than found.`,
  parameters: {
    type: "OBJECT",
    properties: {
      description: {
