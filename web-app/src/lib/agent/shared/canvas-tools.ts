...
export const SET_PAGE_BACKGROUND: ToolDeclaration = {
  ...
  description: `Paint one page of a board a colour, or take its colour off. This is how "make that page black", "give it a warm background", "put it back on white" are done, and it is how a page gets a flat colour ground: a page's colour is the page's own, so it is never a rectangle placed on top of one — a rectangle you draw is an object that can be moved, restacked and picked up by accident, and this is not. It is one of two kinds of ground a page can have — the other is a picture laid to cover the page and sent to the back, and where nothing has chosen between them, a pictured ground usually serves a page better than a flat colour. Nothing on the page moves and nothing is taken off: the ground goes behind everything already standing there, which is worth thinking about before you paint, because near-black lettering on a page painted near-black is a page that looks emptied without anything having left it. Read the board with read_canvas first — pages are told apart by an id, the wrong page is somebody else's work, and each page there says the colour it already stands on. A page already that colour is left alone and said so, and painting a second colour repaints the page rather than stacking one ground on another.`,
  parameters: {
    type: "OBJECT",
    properties: {
      boardId: {
